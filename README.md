...
| **이승엽 (Infra)** |
| :--- |
| • 개발 및 운영 환경 격리 (Docker): 독립적인 가상 네트워크로 운영과 개발/테스트 환경 분리 |
| • 무중단 배포 구축: Jenkins Pipeline과 Blue-Green 방식을 적용한 안정적인 CI/CD 환경 마련 |
| • API 리팩토링: 기존 방 생성 및 OpenVidu 세션 초기화를 통합해 API 호출 최적화 및 데이터 정합성 확보 |
| • UX 고도화: STOMP 소켓 API를 활용해 진행 상태를 시각화하고, 전원 완료 시 다음 페이즈로 전환되는 동기화 로직 구현 등 |
| • 배포 알림 자동화: Mattermost Webhook 연동을 통한 실시간 배포 알림 파이프라인 구축 |
<br>
