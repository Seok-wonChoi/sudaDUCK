...
export default function SentenceCard({
  ...
  const handleBookmark = () => {
    ...
    // console.log('ë¶ë§ˆ??? ê?:', sentenceId, newBookmarkedState);
  };
  ...
  const getDisplayEnglish = () => {
    ...
    // AIê°€ ?½ëŠ” ì¤‘ì´ê±°ë‚˜ ?¹ìŒ ?€ê¸?ì¤‘ì—???„ì²´ ë¬¸ì¥ ë³´ì—¬ì£¼ê¸°
          ...
          splits.forEach((split, idx) => {
            if (split.toLowerCase() === word.toLowerCase()) {
              if (showBlanks) {
                ...
                // ë¹ˆì¹¸ ëª¨ë“œê°€ êº¼ì ¸?ˆì„ ?ŒëŠ” ?ìŠ¤?¸ë? ë³´ì—¬ì£¼ë˜ ê°•ì¡° ?œì‹œ
                newParts.push(
          ...
          <div className={styles.recordingActive}>
            <div className={styles.recordingStatus}>
              <div className={styles.statusColumn}>
                ...
                <span className={styles.statusTextLarge}>? ì‹œ ???¹ìŒ???œì‘?©ë‹ˆ??/span>
                <span className={styles.recordingHintLarge}>?ì–´ë¡??½ì„ ì¤€ë¹?!! ?™ï¸?/span>
              </div>
            </div>
          </div>
        );
            ...
            <div className={styles.recordingActive}>
              ...
              <div className={styles.sideArea}>
                <div className={styles.switchWrapper}>
                  <span className={styles.switchLabel}>ë¹ˆì¹¸</span>
                  <button 
                    type="button" 
                    className={`${styles.iosSwitch} ${showBlanks ? styles.switchOn : ""}`}
                    onClick={onToggleBlanks}
                    disabled={isSubmitting}
                    aria-label={showBlanks ? "ë¹ˆì¹¸ ?„ê¸°" : "ë¹ˆì¹¸ ì¼œê¸°"}
                  >
                ...
                {onStop && (
                  ...
                  >
                    ...
                    <span className={styles.stopText}>
                      {isSubmitting ? '?‰ê? ì¤? : '?ë‚´ê¸?}
                    </span>
                  </button>
                )}
              </div>
            </div>
            <p className={styles.recordingHintText}>ë¬¸ì¥??ì²œì²œ???ë°•?ë°• ?°ë¼ ë§í•´ë³´ì„¸??</p>
          </div>
        );
          ...
          <div className={styles.recordingStatus}>
            {isSubmitting ? (
              <div className={styles.submittingStatus}>
                ...
                <span className={styles.statusText}>?‰ê? ?„ì†¡ ì¤?..</span>
              </div>
            ) : (
              <div className={styles.statusColumn}>
                ...
                <span className={styles.nextSentenceHint}>?¤ìŒ ë¬¸ì¥?¼ë¡œ..</span>
              </div>
            )}
          </div>
        );
    ...
    <div className={`${styles.sentenceCard} ${isActive ? styles.active : ''} ${getScoreGradeClass()}`}>
      <div className={styles.header}>
        <div className={styles.speakerInfo}>
          <span className={styles.speakerName}>?™ï¸?ë°œí™”??: {speaker}</span>
        </div>
        <div className={styles.headerRight}>
          ...
          {score !== null && cardState === 'idle' && (
            <button
              ...
              aria-label={isBookmarked ? "?€???´ì œ" : "?€?¥í•˜ê¸?}
              title={isBookmarked ? "?€???´ì œ" : "?€?¥í•˜ê¸?}
            >
              <img
      ...
      <div className={styles.content}>
        <div className={styles.section}>
          <span className={styles.label}>?œêµ­??/span>
